...
public class Calculator {
	...
	private static String decideOperation(String input) {
		try{
			...
			if(input.startsWith("LET")){
				...
				letParameters = findLetParams(input);
	...
	*/
	 ...
	 */
	public static List<String> findLetParams(String input) {
		try{
			...
			{
				exp2EndPos = findPosition(input, exp2StartPos);			
	...
	private static int findPosition(String input, int startPos) {
		try{
	 ...
	 */
	public static String operate(String input, Stack<String> myStack) {
		String error = "";
		try{
				...
				if(input.charAt(i) == ')'){
					...
					if(myStack.isEmpty()){
						...
						case "ADD":
							...
							result = Double.valueOf((Integer.parseInt(s1)) + Double.valueOf(Integer.parseInt(s2)));
							break;
						case "SUB":
